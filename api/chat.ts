...
export default async function handler(req: any, res: any) {
  // Polyfill express-like res.status and res.json helpers if they don't exist
...
- Always maintain hospitality, polite Ugandan vibes, and conciseness.`;
    ...
    console.warn("OpenAI Chat serverless function failed, deploying Kampala Sandbox fallback:", err.message || err);
    
    // Attempt fallback response
    let lastMsg = '';
    try {
      const body = await parseBody(req);
      lastMsg = (body.message || '').toLowerCase();
    } catch (e) {}
